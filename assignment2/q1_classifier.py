...
class SoftmaxModel(Model):
    ...
    def add_placeholders(self):
        ...
        """
        ### YOUR CODE HERE
        self.input_placeholder = tf.placeholder(dtype=tf.float32, shape=(None, self.config.n_features), name="input")
        self.labels_placeholder = tf.placeholder(dtype=tf.int32, shape=(None, self.config.n_classes), name="labels")
        ### END YOUR CODE
    ...
    def add_prediction_op(self):
        ...
        """
        ### YOUR CODE HERE
        Weights = tf.Variable(np.zeros(shape=(self.config.n_features, self.config.n_classes)), 
                              dtype=tf.float32, name="Weights")
        bias = tf.Variable(np.zeros(shape=self.config.n_classes), dtype=tf.float32, name="bias")
        yhat = tf.matmul(self.input_placeholder, Weights) + bias
        pred = softmax(yhat)
        ### END YOUR CODE
        return pred
    ...
    def add_training_op(self, loss):
        ...
        """
        ### YOUR CODE HERE
        train_op = tf.train.GradientDescentOptimizer(learning_rate=self.config.lr).minimize(self.loss)
        ### END YOUR CODE
        return train_op
