...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    for sentence, labels in data:
        ...
        for word_id in range(len(sentence)):
            word_features = []
            for shift_id in range(-window_size, window_size+1, 1):
                if shift_id + word_id < 0:
                    word_features.extend(start)
                elif shift_id + word_id >= len(sentence):
                    word_features.extend(end)
                else:
                    word_features.extend(sentence[shift_id + word_id])
            windowed_data.append((word_features, labels[word_id]))
    	### END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (!3-5 lines)
        self.embedding_matrix = tf.Variable(self.pretrained_embeddings, name="embedding_matrix")
        embeddings = tf.nn.embedding_lookup(params=self.embedding_matrix, ids=self.input_placeholder)                                      
    ...
    def add_prediction_op(self):
        ...
        W_h = tf.get_variable(name="W_h", shape=[x.get_shape()[-1], self.config.hidden_size], 
                              initializer=tf.contrib.layers.xavier_initializer())
        b_h = tf.get_variable(name="b_h", shape=[self.config.hidden_size], 
                              initializer=tf.zeros_initializer())
        h = tf.nn.relu(tf.matmul(x, W_h) + b_h, name="h")
        h_drop = tf.nn.dropout(h, keep_prob=self.dropout_placeholder, name="h_drop")
        
        W_u = tf.get_variable(name="W_u", shape=[self.config.hidden_size, self.config.n_classes], 
                              initializer=tf.contrib.layers.xavier_initializer())
        b_u = tf.get_variable(name="b_u", shape=[self.config.n_classes], 
                              initializer=tf.zeros_initializer())
        pred = tf.matmul(h_drop, W_u) + b_u
        ### END YOUR CODE
        return pred
